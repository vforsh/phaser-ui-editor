...
export class Phaser3Extensions {
	public static extend(): void {
		if (window.__Phaser3ExtensionsApplied) {
			return
		}

		window.__Phaser3ExtensionsApplied = true
	...
	public static extendGraphics(): void {
		Phaser.GameObjects.Graphics.prototype.inverseSlice = function (
			x: number,
			y: number,
			radius: number,
			startAngle: number,
	...
	private static extendLoader() {
		...
		Phaser.Loader.LoaderPlugin.prototype.bitmapFontFromAtlas = function (
			...
			this.once(Phaser.Loader.Events.COMPLETE, () => {
				let wasParsed: boolean = Phaser.GameObjects.BitmapText.ParseFromAtlas(
					this.scene,
					fontKey,
					atlasKey,
					atlasFrame,
	...
	private static extendEventEmitter() {
		// we need to extend eventemitter3 for this to work properly
		...
		EE.has = function (event, fn, context): boolean {
			...
			return arrayify(listeners).some((listener) => {
				if (context) {
		...
		EE.ensureOn = function (event, fn, context): boolean {
			if (this.has(event, fn, context)) {
				return false
			}
		...
		EE.ensureOnce = function (event, fn, context): boolean {
			if (this.has(event, fn, context)) {
				return false
			}

			this.once(event, fn, context)
		...
		EE.offByContext = EE.removeByContext
	}
}
